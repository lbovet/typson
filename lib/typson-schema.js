...
(function (underscore, Q, _traverse, typson) {
    ...
    var validationKeywords = [ "type", "minimum", "exclusiveMinimum", "maximum", "exclusiveMaximum", "multipleOf", "minLength", "maxLength", "format", "pattern", "minItems", "maxItems", "uniqueItems", "additionalProperties" ];
    ...
    /**
     * Extracts the default value stored in a comment and registers it on property.
     *
     * @param comment {string} the full comment.
     * @param to {object} the destination variable.
     */
    function copyPropertyDefault(comment, to) {
        comment = comment.replace(/@default\s+([^@]+)/, function(m, m1) {
            to.default = m1.trim();
            return "";
        });
    }
    ...
    function copyValidationKeywords(comment, to) {
        copyPropertyDefault(comment, to);
