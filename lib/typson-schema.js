...
(function (underscore, Q, _traverse, typson) {
    ...
    var schemaTypeToOptions = {
        portal: {
            ...
            annotedValidationKeywordPattern: /(@|@_)[a-z0-9A-Z\'\.]+([^\S\r\n].*|\s|$)/gi,
    ...
    function copyValidationKeywords(comment, to, schemaTypeDefaults) {
        ...
        while ((annotation = schemaTypeDefaults.annotedValidationKeywordPattern.exec(comment))) {
            ...
            var keyword = annotationTokens[0].slice(1).trim();
            ...
            if (schemaTypeDefaults.removeDefaults && keyword === "default") {
                continue;
            }
            if (annotationTokens[1] && schemaTypeDefaults.requiredAnnotation === keyword) {
                to.required = annotationTokens[1].split(",");
                continue;
            }
            if (annotationTokens[1] && schemaTypeDefaults.requirementAnnotations.indexOf(keyword) !== -1) {
