...
(function (underscore, Q, _traverse, typson) {
    ...
    var validationKeywords = [ "type", "minimum", "exclusiveMinimum", "maximum", "exclusiveMaximum", "multipleOf", "minLength", "maxLength", "format", "pattern", "minItems", "maxItems", "uniqueItems", "default", "additionalProperties", "_requiredIfAvailable" ];
    var annotedValidationKeywordPattern = /@_requiredIfAvailable|(@|@_)[a-z0-9A-Z\']+\s.*|(@_requiredIfAvailable|@default|@maxLength)/gi;
    ...
    function handlePropertyDeclaration(type, definition, definitions, refPath) {
        ...
        _.each(type.members.members, function (variable) {
            ...
            //maps
                ...
                if (property.id.toLowerCase() == "GeneralPayloadMap".toLowerCase()) {
                    console.log("GeneralPayloadMap");
                }
    ...
    function mergeInheritedProperties(type, definition, definitions) {
        if (type.extendsList) {
            _.each(type.extendsList.members, function (superType) {
                ...
                if (superDefinition) {
                    // recursive call
                    mergeInheritedPropertes(superType, definition, definitions);
                    // merges properties
    ...
    var LEGAL_VERTICALS = ["physical", "digital", "marketPlace", "services", "vgolds", "onePlus", "sammyDress"];
    /**
    ...
    function copyValidationKeywords(comment, to) {
        ...
        while ((annotation = annotedValidationKeywordPattern.exec(comment))) {
            ...
            // @_requiredIfAvailable -> add as empty string
            if (keyword === "_requiredIfAvailable") {
                to._requiredIfAvailable = "";
                continue;
            }
            // @_hiddenInVerticals -> add as array
            if (keyword === "_hiddenInVerticals") {
                var verticals = annotationTokens[1].split(",");
                var illegalValues = _.difference(verticals, LEGAL_VERTICALS);
                if (illegalValues.length > 0) {
                    throw new Error("Illegal verticals " + illegalValues);
                }
                to._hiddenInVerticals = verticals;
                continue;
            }
            ...
            if (validationKeywords.indexOf(keyword) >= 0) {
                ...
                if ((keyword === "default" || keyword === "pattern") && value ) {
